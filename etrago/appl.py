...
args = {
    ...
    "db": "local2035",  # database session #smh. the default value was "egon-data" which here I changed to "local2035" to make it similar to Clara's json file
    "gridversion": None,  # None for model_draft or Version number
    "method": {  # Choose method and settings for optimization
        "type": "lopf",  # type of optimization, currently only 'lopf' smh."name of "market_grid" this market is seems to be ore relevant to my work,
        "n_iter": 1,  # abort criterion of iterative optimization, 'n_iter' or 'threshold' smh. it could be set to 4 also but 1 will keep the process shorter
        "pyomo": True,  # set if pyomo is used for model building
    },
    "pf_post_lopf": {
        "active": True,  # choose if perform a pf after lopf #smh. this could be marked as false
    ...
    "csv_export": "Result_V1_8760_50cl_test_",  # save results as csv: False or /path/tofolder smh. looking for the result
    # Settings:
    "extendable": {
        "extendable_components": [
            "as_in_db" #smh. as it is in the database here we set what we want to optimize it will normaly takes everythin from the database
        ],  # Array of components to optimize
        "upper_bounds_grid": {  # Set upper bounds for grid expansion
            # lines in Germany
            "grid_max_D": None,  # relative to existing capacity smh. read below comments, but instead of None here we can easily extend the total capacity to for exampe "4" or anything thing else
            "grid_max_abs_D": {  # absolute capacity per voltage level #smh. using below will create upper buound incase if required change the circutes numer 
...
pandas             2.2.0
    ...
    # adjust network regarding eTraaGo setting
    ...
    # snapshot clustering
    etrago.snapshot_clustering() #we set it fo false so it will skip it

    # skip snapshots
    etrago.skip_snapshots() #we set it fo false so it will skip it

    # start linear optimal powerflow calculations
    etrago.lopf() #soe of the there belwo function might not be required for my master thesis???
...
if __name__ == "__main__":
    ...
    etrago = run_etrago(args, json_path=None) #smh. let it to be None. the argument will takes value from the default file, or you can set the json directory here manually here. The default file is nearby to appl.py. Consider in the json file that the start and end snapshop is important. to reduce time consumption, define values to 1 and 4 hour . after setting all you can run #the the appl.py using spyder
    print(datetime.datetime.now())
...
# smh. also there is a file with name of config.ini which is .etrago_database and this will provide configuration to have to connect to the database.
#smh. there will be two way to solve the connection error: updating the json file and copy paste them in our file, and or also check the parameter of the Clara file with what we have it. the reult will be in lopf_itreration foler. theree will be many file inside of this  vgy67
