...
def add_ch4_constraints(self, network, snapshots):
    print("CH4-P1 start constraints") # 30 time repetation
    ...
    print("CH4-P1 end constraints") # 30 time repetation

def add_ch4_constraints_nmp(self, network, snapshots):
    print("CH4-P2 end constraints") # 30 time repetation
...
class Constraints:
    ...
    def functionality(self, network, snapshots):
        ...
        if "power_to_Heat" in network.links.carrier.values:
            # print('WWTP')
            add_heat_constraint(network, snapshots)

        if "power_to_O2" in network.links.carrier.values:
            add_o2_constraint(network, snapshots)
...
def add_heat_constraint(network, snapshots):
    # Define the reference links which all other links need to follow its power
    power_to_h2_links = network.links.index[network.links.carrier == "power_to_H2"]
    
    # Define the AC bus value in the above links to distinguish further connections
    power_to_h2_bus = network.buses.loc[network.links.loc[power_to_h2_links, "bus0"]].index.unique()

    # Circulate to find the desired links which have connection with reference bus
    for i in power_to_h2_bus:
        power_to_heat_links = network.links[
            (network.links.carrier == "power_to_Heat") & (network.links.bus0 == i)].index
        power_to_h2_links1 = network.links[
            (network.links.carrier == "power_to_H2") & (network.links.bus0 ==i)].index
        # print(f'Heat: {wasteheat_to_power_links}, Power: {power_to_h2_links1}')
        # if len(wasteheat_to_power_links) == 0:
        #     continue  # Skip if no connected links

        def heat_to_power_ratio(model, snapshot):
            # Get the efficiency for the wasteheat_to_power links
            heat_ratio = 0.2 # float(network.links.loc[power_to_heat_links, "type"].iloc[0])
            # Calculate the lhs and rhs of the constraint
            lhs_power_to_heat = sum(model.link_p_nom[pth_idx] for pth_idx in power_to_heat_links)
            rhs_power_to_heat = heat_ratio * sum(model.link_p_nom[pth2_idx] for pth2_idx in power_to_h2_links1)
            # print(f'LHS: {lhs_wasteheat_to_power}, RHS: {rhs_wasteheat_to_power}')
            return lhs_power_to_heat <= rhs_power_to_heat

        setattr(
            network.model,
            f"heat_to_power_ratio{i}",
            Constraint(list(snapshots), rule=heat_to_power_ratio),
        )

def add_o2_constraint(network, snapshots):
    # Define the reference links which all other links need to follow its power
    power_to_h2_links = network.links.index[network.links.carrier == "power_to_H2"]
    ...
    for i in power_to_h2_bus:
        power_to_o2_links = network.links[
            (network.links.carrier == "power_to_O2") & (network.links.bus0 == i)].index
        power_to_h2_links1 = network.links[
            (network.links.carrier == "power_to_H2") & (network.links.bus0 ==i)].index
        # print(f'Heat: {o2_to_power_links}, Power: {power_to_h2_links1}')
        # if len(wasteheat_to_power_links) == 0:
        #     continue  # Skip if no connected links

        def o2_to_power_ratio(model, snapshot):
            # Get the efficiency for the wasteheat_to_power links
            o2_ratio = 0.05 # float(network.links.loc[power_to_o2_links, "type"].iloc[0])
            # Calculate the lhs and rhs of the constraint
            lhs_power_to_o2 = sum(model.link_p_nom[pto2_idx] for pto2_idx in power_to_o2_links)
            rhs_power_to_o2 = o2_ratio * sum(model.link_p_nom[pth2_idx] for pth2_idx in power_to_h2_links1)
            # print(f'LHS: {lhs_wasteheat_to_power}, RHS: {rhs_wasteheat_to_power}')
            return lhs_power_to_o2 <= rhs_power_to_o2

        setattr(
            network.model,
            f"o2_to_power_ratio{i}",
            Constraint(list(snapshots), rule=o2_to_power_ratio),
        )